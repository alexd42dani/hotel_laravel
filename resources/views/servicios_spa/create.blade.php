...
@section('title','Servicios spa agregar')
...
@section('header', 'Servicios spa agregar')

@section('content')

<form onsubmit="return validateForm()" method="POST" action="{{route('servicios_spa.store')}}">
    @csrf
    ...
    <div class="container" style="margin-top:30px">
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="form-group">
                    <label for="spa">Spa o Sauna</label>
                    <input type="text" id="spa" class="readonly" autocomplete="off" style="caret-color: transparent !important;">
                    <input type="button" value="..." class=" btn-dark" onclick="openWin('spa')">
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-10 form-group">
                <div class="table-responsive-md table-hover from-group" style="overflow-y:auto; height:200px">
                    <table id="spa_table" data-toggle="table" data-classes="table table-bordered table-hover table-md">
                        <thead>
                            <tr>
                                <th>Id</th>
                                <th>Spa o Sauna</th>
            ...
            <div class="col-md-2 form-group">
                <div class="col-md-12">
                    <button type="button" class="btn btn-dark form-group" onclick="add('spa')" tabindex="8">Agregar</button>
                </div>
                <div class="col-md-12">
                    <button type="button" class="btn btn-dark form-group" onclick="remove('spa')">Quitar</button>
                </div>
                <div class="col-md-12">
                    <button type="button" class="btn btn-dark form-group" style="display: none;" onclick="edit('spa')">Editar</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function add(table) {
        ...
        if (table === "spa") {
            ...
            var val = parseInt(document.getElementById("spa").value);
            if (can === "" || val === "") {
                alert("Cargar campos primero");
                //console.log(x);
                return false;
            ...
            // var val = parseInt(document.getElementById('tarifa').value);
            //var path = "empty";
            var path = "{{route('servicios_spa.sauna')}}";
            var table = "#spa_table"
            //console.log(x);
            // addTable(x, table);
        }
        ...
        function addTable(data, table) {
            ...
            if (table === "#spa_table") {
                ...
                var name_articulo = document.getElementsByName("spa_detalle[]");
                for (let index = 0; index < name_articulo.length; index++) {
                    const element = name_articulo[index];
                    if (element.value == data.spas[0].codigo) {
                        alert("Codigo ya ingresado");
                        return false;
                    }
                }
                ...
                $(table).bootstrapTable('insertRow', {
                    index: 0,
                    row: [data.spas[0].id, data.spas[0].nombre, can, data.spas[0].precio.format(0, 3, '.', ',')+
                        '<input type="hidden" name ="spa_detalle[]" value=' + data.spas[0].id + '>' +
                        '<input type="hidden" name ="cantidad_detalle[]" value=' + parseInt(can) + '>' 
                    ]
                })
                
                total += parseInt(parseInt(can) * data.spas[0].precio);
    ...
    var id_delete = null;
    $('#huesped_table, #spa_table').on('click-row.bs.table', function(e, row, $element, field) {
        id_delete = row;
    ...
    $('#huesped_table, #spa_table, #habitacion_huespedes').on('click', 'tbody tr', function(event) {
        $(this).addClass('highlight').siblings().removeClass('highlight');
    });

    function remove_all() {
        $("#huesped_table").bootstrapTable('removeAll');
        $("#spa_table").bootstrapTable('removeAll');
    ...
    function openWin(w) {
        ...
        if (w == "spa") {
            myWindow = window.open("{{ route('searcher.spa') }}", "_blank", "width=1000, height=500, menubar=no, top=50, left=250");
        }
        if (w == "requisicion") {
    ...
    function validateForm() {
        ...
        var taf1 = document.getElementsByName("spa_detalle[]");
        if (taf1.length == 0) {
