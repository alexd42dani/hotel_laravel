...
<div class="container" style="margin-top:30px">
    <div class="row">
        <div class="col-md-10 form-group">
            <div>
                ...
                <thead>
                    ...
                    <tbody>
                        @foreach($recaudaciones as $recau)
                        <tr>
                            <td>{{$recau->id}}</td>
                            <td>{{$recau->entidad}}</td>
                            <td>{{$recau->numero_boleta}}</td>
                            <td>{{number_format($recau->monto,0,'','.')}}</td>
                            <td>{{$recau->tipo_de_deposito}}</td>
                            <td>{{ \Carbon\Carbon::parse($recau->fecha)->format('d/m/Y')}}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="col-md-2 form-group mt-5">
            <div class="col-md-12">
                <a class="btn btn-dark form-group" href="{{ route('recaudaciones.create') }}">Agregar</a>
            </div>
            <div class="col-md-12">
                <a id="anular" class="btn btn-dark form-group" onclick="return mensaje_anular()" href="{{ route('recaudaciones.destroy', 'empty') }}">Eliminar</a>
            </div>
            <div class="col-md-12">
                <a id="modificar" style="display: none;" class="btn btn-dark form-group" onclick="return mensaje_modificar()" href="{{ route('recaudaciones.edit', 'empty') }}">Modificar</a>
            </div>
        </div>
    </div>
</div>
