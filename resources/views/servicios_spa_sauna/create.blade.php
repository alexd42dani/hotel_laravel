...
@section('title','Servicios spa y sauna agregar')
...
@section('header', 'Servicios spa y sauna agregar')

@section('content')

<form onsubmit="return validateForm()" method="POST" action="{{route('servicios_spa_sauna.store')}}">
    @csrf
        ...
        <div class="row">
            <div class="col-md-10">
                <div class="d-flex flex-wrap justify-content-md-around ">
                    ...
                    <div class="p-2 mx-auto"><button type="button" onclick="location.href = '{{ route('servicios_spa_sauna.index') }}'" class="btn btn-dark">SALIR</button></div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function add(table) {
        ...
        if (table === "consumicion") {
            ...
            var val = [];
            val[0] = parseInt(document.getElementById("spa").value);
            ...
            // var val = parseInt(document.getElementById('tarifa').value);
            //var path = "empty";
            var path = "{{route('servicios_spa_sauna.spa_sauna')}}";
        ...
        function addTable(data, table) {
            ...
            if (table === "#consumicion_table") {
                ...
                var name_articulo = document.getElementsByName("spa_sauna_detalle[]");
                for (let index = 0; index < name_articulo.length; index++) {
                    const element = name_articulo[index];
                    const element1 = document.getElementsByName("habitacion_detalle[]")[index];
                    if (element.value == data.spa_sauna[0].id && element1.value == data.habitaciones[0].id) {
                        alert("Habitacion y spa/sauna ya ingresado");
                        return false;
                    }
                }
                ...
                $(table).bootstrapTable('insertRow', {
                    index: 0,
                    row: [data.spa_sauna[0].id, data.spa_sauna[0].nombre, can, data.spa_sauna[0].precio.format(0, 3, '.', ','),
                        data.habitaciones[0].descripcion, data.huespedes[0].nombre + " " + data.huespedes[0].apellido, promocion+"%" +
                        '<input type="hidden" name ="spa_sauna_detalle[]" value=' + data.spa_sauna[0].id + '>' +
    ...
    function validateForm() {
        var taf1 = document.getElementsByName("spa_sauna_detalle[]");
        if (taf1.length == 0) {
