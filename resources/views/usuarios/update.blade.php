...
@section('header', 'Usuarios editar')

@section('content')

<form onsubmit="return validateForm()" method="POST" action="{{route('usuarios.update',$variables[0]->id)}}">
    @csrf
    @method('PUT')
        ...
        <div class="row">
            <div class="col-md-4">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="empleado" name="empleado" class="readonly" value="{{$variables[0]->empleado_id}}" required autocomplete="off" style="caret-color: transparent !important;">
                    <input type="button" value="..." class=" btn-dark" onclick="openWin('empleado')">
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    <label for="nombre">Usuario</label>
                    <input type="text" id="nombre" name="nombre" required value="{{$variables[0]->usuario}}">
                </div>
                <div class="form-group">
                    <label for="nivel">Nivel de usuario</label>
                    <select id="nivel" name="nivel" required>
                        <option value="">--Selecciona una opción--</option>
                        <option value="ADMIN" {{ $variables[0]->nivel_usuario=="ADMIN" ? 'selected' : '' }}>ADMIN</option>
                        <option value="RECEPCION" {{ $variables[0]->nivel_usuario=="RECEPCION" ? 'selected' : '' }}>RECEPCION</option>
                        <option value="CAJERO" {{ $variables[0]->nivel_usuario=="CAJERO" ? 'selected' : '' }}>CAJERO</option>
                    </select>
                </div>
            </div>
            <div class="col-md-4">
            ...
            <div class="row">
                <div class="col-md-10">
                    <div class="d-flex flex-wrap justify-content-md-around ">
                        ...
                        <div class="p-2 mx-auto"><button type="button" onclick="location.href = '{{ route('area.index') }}'" class="btn btn-dark">SALIR</button></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function openWin(w) {
        if (w == "empleado") {
            myWindow = window.open("{{ route('searcher.empleado') }}", "_blank", "width=1000, height=500, menubar=no, top=50, left=250");
        }
        if (w == "reserva") {
    ...
    function play() {
        ...
        if (empleado != "nothing" && empleado != null) {
